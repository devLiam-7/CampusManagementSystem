...
class ViewMarks extends BaseFrame{
    ...
    private void initComponents(){
        ...
        roll = new JTextField();
        roll.setFont(new Font("Segoe UI", Font.BOLD, 13));
        roll.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(new Color(203, 213, 225)),
                BorderFactory.createEmptyBorder(5, 10, 5, 10)
        ));
        roll.setPreferredSize(new Dimension(200, 35));
        roll.putClientProperty("JTextField.placeholderText", "Enter Roll No");
        ...
        view.addActionListener(e -> loadMarks(roll.getText().trim()));
        
            topBar.add(roll, BorderLayout.CENTER);
    ...
    if (search == null || search.isEmpty()) return;
        ...
        try {
            Connection con = DBConnection.getConnection();
             String sql = "SELECT s.roll_no, subject, marks, max_marks, grade " +
             "FROM marks a JOIN students s ON a.roll_no = s.roll_no " +
             "WHERE a.roll_no = ?";
            ...
            while (rs.next()) {
                ...
                tableModel.addRow(new Object[]{subject, marks, maxMarks, grade});
            }
            con.close();
        } catch (Exception e) {
